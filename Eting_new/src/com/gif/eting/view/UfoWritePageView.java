...
public class UfoWritePageView extends View {
	...
	public UfoWritePageView(Context context, AsyncTaskCompleteListener<String> callback) {
		super(context);
		this.callback = callback;

		// 이미지를 불러온다.
		mDrawable = CachedBitmap.getIntro_ufo(context);
		// 이미지 너비
		...
		// 한쪽 끝
		this.p1 = new Point(Util.getX(50), Util.getY(43));
		// 다른쪽 끝
		this.p2 = new Point(Util.getX(50), Util.getY(47));
		// 현재위치
		this.p = new PointF(Util.getX(50), Util.getY(45));
		// 한번의 애니메이션을 표시하는데 걸리는 시간.
		this.duration = 1000 / Const.fps;
